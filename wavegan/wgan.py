...
def compute_discr_loss_terms(model_dis, model_gen, real_data_v, noise_v,
    ...
    D_real = model_dis(real_data_v, label)
    D_real = D_real.mean()
    ...
    fake = autograd.Variable(model_gen(noise_v, label).data)
    inputv = fake
    D_fake = model_dis(inputv, label)
    D_fake = D_fake.mean()
    if compute_grads:
        D_fake.backward(one)
...
def compute_gener_loss_terms(model_dis, model_gen, batch_size, latent_dim, num_classes,
                             use_cuda, compute_grads=False):
    # if not isinstance(label, torch.LongTensor) and not isinstance(label, torch.cuda.LongTensor):
    #     label = torch.tensor(label).long()
    # if use_cuda:
    #     label = label.cuda()
    label = torch.randint(num_classes, (1, batch_size))
    label = label.view(-1)
    if use_cuda:
        label = label.cuda()
    # Convenient values for
...
def calc_gradient_penalty(model_dis, real_data, fake_data, batch_size, lmbda, use_cuda=True, label=None):
    ...
    disc_interpolates = model_dis(interpolates, label)
...
def train_wgan(model_gen, model_dis, train_gen, valid_data, test_data,
               ...
               sample_size=20, lr=1e-4, beta_1=0.5, beta_2=0.9, latent_dim=100, display_interval=1, model_dir='.'):
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx in range(batches_per_epoch):
            ...
            for iter_d in range(discriminator_updates):
                ...
                real_data_v = np_to_input_var(data_dict['X'], use_cuda)
                real_data_c = data_dict['Y']
                ...
                batch_history['discriminator'].append({
                    ...
                    'cost': D_cost_train.item(),
                    'wasserstein_cost': D_wass_train.item(),
                    'cost_validation': D_cost_valid.item(),
                    'wasserstein_cost_validation': D_wass_valid.item(),
                })
            ...
            batch_history['generator']['cost'] = G_cost.item()
        ...
        if (epoch + 1) % display_interval == 0:
            model_gen_output_path = os.path.join(model_dir, "model_gen_{}.pt".format(epoch + 1))
            model_dis_output_path = os.path.join(model_dir, "model_dis_{}.pt".format(epoch + 1))
            torch.save(model_gen, model_gen_output_path)
            torch.save(model_dis, model_dis_output_path)
    ...
    # Get new batch of real training data
    test_label = np_to_input_var(np.resize(test_data['Y'], (-1, 1)), use_cuda)
    D_cost_test, D_wass_test = compute_discr_loss_terms(
        model_dis, model_gen, test_data_v, noise_v, batch_size, latent_dim, test_label,
        lmbda, use_cuda, compute_grads=False)

    valid_label = np_to_input_var(np.resize(valid_data['Y'], (-1, 1)), use_cuda)
    D_cost_valid, D_wass_valid = compute_discr_loss_terms(
        model_dis, model_gen, valid_data_v, noise_v, batch_size, latent_dim, valid_label,
        lmbda, use_cuda, compute_grads=False)
    ...
    final_discr_metrics = {
        ...
        'cost_validation': D_cost_valid.item(),
        'wasserstein_cost_validation': D_wass_valid.item(),
        'cost_test': D_cost_test.item(),
        'wasserstein_cost_test': D_wass_test.item()
    }
