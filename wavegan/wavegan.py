...
class WaveGANGenerator(nn.Module):
    def __init__(self, model_size=64, ngpus=1, num_channels=1, latent_dim=100,
                 post_proc_filt_len=512, verbose=False, upsample=True, num_class=0):
        ...
        self.fc1 = nn.DataParallel(nn.Linear(latent_dim // 3 * 4, 256 * model_size * 2))
        self.tconv1 = None
        self.tconv2 = None
        self.tconv3 = None
        self.tconv4 = None
        ...
        if self.num_class != 0:
            self.class_embedding = nn.Embedding(self.num_class,  latent_dim // 3)
    
        if self.upsample:
            self.upSampConv0 = nn.DataParallel(
...
class WaveGANDiscriminator(nn.Module):
    def __init__(self, model_size=64, ngpus=1, num_channels=1, shift_factor=2, alpha=0.2, batch_shuffle=False, verbose=False, num_class=1):
        ...
        self.verbose = verbose
        self.num_class = num_class
        self.class_embedding = nn.Embedding(self.num_class, self.model_size * 64)
        ...
        self.ps5 = PhaseShuffle(shift_factor, batch_shuffle=batch_shuffle)

        self.fc1 = nn.DataParallel(nn.Linear(512 * model_size + 64 * self.model_size, 1))
