...
default_params = {
    ...
    'loss_smoothing': 0.99,
    'cuda': True,
    'comet_key': "parameter of comet plugin",
    'project_name': "parameter of comet plugin",
    'workspace': "parameter of comet plugin"
}
...
class_label = ['airport', 'tram', 'shopping_mall', 'metro_station', 'public_square', 'street_pedestrian', 'metro', 'park', 'bus', 'street_traffic']
...
def load_last_checkpoint(checkpoints_path, load_best=False):
    # checkpoints_pattern = os.path.join(
    #     checkpoints_path, SaverPlugin.last_pattern.format('*', '*')
    # )

    checkpoint_paths = [os.path.join(checkpoints_path, filename) for filename in os.listdir(checkpoints_path)]
...
def make_data_loader(overlap_len, params):
    ...
    path = params['datasets_path']
    def data_loader(split_from, split_to, eval):
        dataset = FolderDataset(
            path, overlap_len, params['q_levels'], split_from, split_to, sample_rate=params['sample_rate']
        )
        return DataLoader(
            dataset,
...
def init_comet(params, trainer):
    if params['comet_key'] is not None:
        ...
        experiment = Experiment(api_key=params['comet_key'], log_code=True, project_name=params['project_name'], workspace=params['workspace'])
        hyperparams = {
            name: param_to_string(params[name]) for name in tag_params
        }
        # experiment.log_multiple_params(hyperparams)
        experiment.log_parameters(hyperparams)
        trainer.register_exp(experiment)
...
def main(exp, frame_sizes, dataset, **params):
    ...
    for i in range(0, 10):
        y = generator(1000, 160000, i, data_seed=data_loader(val_split, test_split, eval=True).dataset).float().numpy()
        for j in range(len(y)):
            path = os.path.join(save_path, '{}_{}.wav'.format(class_label[i], j))
